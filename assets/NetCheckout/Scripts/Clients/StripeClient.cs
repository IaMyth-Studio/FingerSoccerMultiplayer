...
namespace NetCheckout
{
    ...
    public class StripeClient : BaseClient, ICheckoutClient
    {
        ...
        {
            settings.StripeSecretKey = secret;
        }
		...
		/// <summary>
		/// Shortcut function for making purchases with an existing product.
		/// </summary>
		/// <param name="productId">Stripe product id. e.g., prod_OalOeOXcUCDU9K</param>
		/// <param name="onComplete">Callback. If successful, returns true and the order id. If failed, returns false and the error response.</param>
		public static void Buy(string productId, int quantity, action onComplete)
		{
			new StripeClient().CreateOrder(productId, quantity, onComplete);
		}
		...
		/// <summary>
		/// First call to initiate the NetCheckout process of making a purchase with an existing product. 
		/// Creates an order on Stripe's servers that the user must confirm to complete the purchase.
		/// </summary>
		/// <param name="productId">Stripe product id. e.g., prod_OalOeOXcUCDU9K</param>
		/// <param name="onComplete">Callback. If successful, returns true and the session id. If failed, returns false and the error response.</param>
		public void CreateOrder(string productId, int quantity, action onComplete)
		{
			WindowController.Instance.DisplayWindow(
				MessageConfig.orderWindow,
				() =>
				{
					CallApiToRetrievePrice(productId, (success, price) =>
					{
						if (!success)
							onComplete?.Invoke(false, price);
						else
							CreateCheckoutSession((Price)price, quantity, "payment", onComplete);
					});
				}
			);
		}
		...
		private void CallApiToRetrievePrice(string productId, action onComplete)
		{
			Request("GET", $"v1/prices?product={productId}&limit=100&active=true", null, (status, res) =>
			{
				object data = IsValid(status) ? JsonUtility.FromJson<ObjectResponse<Price>>(res).data[0] : null;
				onComplete?.Invoke(IsValid(status), data);
			});
		}
        ...
        public void WaitForApproval(string sessionId, action onComplete)
        {
            ...
            (string, string) validator = ("payment_status", "paid");
        ...
        private IEnumerator WaitForApprovalCoroutine(string sessionId, action onComplete)
        {
            ...
            while (waitingOnApproval && WindowController.Instance.Window.IsDisplayed)
            {
                ...
                Request("GET", string.Format("v1/checkout/sessions/{0}", sessionId), null, (status, res) =>
                {
                    ...
                    if (IsValid(status) && !string.IsNullOrEmpty(session.customer) && session.payment_status == "paid")
        ...
        public void Request(string method, string endPoint, List<(string, string)> postData, UnityAction<long, string> callback)
        {
            ...
            request.SetRequestHeader("Stripe-Version", "2020-03-02");
